...
AirFrame *Radio::encapsulatePacket(cPacket *frame)
{
    ...
    airframe->setSenderAngle(getRadioAngle());
...
void Radio::handleLowerMsgStart(AirFrame* airframe)
{
    ...
    double frameAngle = airframe->getSenderAngle();
    ...
    double rcvdPower = receptionModel->calculateReceivedPower(airframe->getPSend(), carrierFrequency, framePos, frameAngle, getRadioPosition(), getRadioAngle());
    if (obstacles && distance > MIN_DISTANCE)
        rcvdPower = obstacles->calculateReceivedPower(rcvdPower, carrierFrequency, framePos, frameAngle, getRadioPosition(), getRadioAngle());
